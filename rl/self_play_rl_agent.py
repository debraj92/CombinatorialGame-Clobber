...
class Agent:
    ...
    def play_move(self, state):
        ...
        new_state = torch.tensor(board + [player]).reshape(1, 1, -1).float()

        return action, reward, action_mask, new_state, done
    ...
    def train_on_one_episode(self, target_model_update, batch_size):
        ...
        fp_state = torch.tensor(board + [player]).reshape(1, 1, -1).float()
        first_round = True

        while not done:
            # First Player Turn
            # We have FP state, reward, action, action_mask
            # We have SP next_state
            fp_action, fp_reward, fp_action_mask, sp_next_state, done = self.play_move(fp_state)
            if done:
                reward = fp_reward
                # If we're done the next state is the terminal state -> None
                sp_next_state = None
            # No second player transition in the first turn
            if first_round:
                first_round = False
            # Save second player transition in memory
            else:
                self.memory.push(sp_state, sp_action, sp_next_state, sp_reward, sp_action_mask)
            sp_state = sp_next_state

            # Second Player Turn
            # We have SP state, reward, action, action_mask
            # We have FP next_state
            if not done:
                sp_action, sp_reward, sp_action_mask, fp_next_state, done = self.play_move(sp_state)
                if done:
                    reward = sp_reward
                    sp_next_state = None
                    # Save the second player transition to memory since
            ...
            # Save first player transition in memory
            self.memory.push(fp_state, fp_action, fp_next_state, fp_reward, fp_action_mask)
